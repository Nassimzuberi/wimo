...
import {addProduct,AddQuantity} from '../../../../cart/store/actions';


const SelectQuantity = ({quantity,cart,id}) => {
    let inCart = (element) => element.id === id;

    let tempo = cart.data.some(inCart) ? 
            (quantity - cart.data[id-1].quantityCart) 
            : quantity;
    const select = [];
...
class Sell extends React.Component {
    ...
    constructor(props){
        ...
        this.add = () => {
            ...
            this.props.cart.data.some(inCart) ? 
